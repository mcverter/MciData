...
from src.PropertyMci.PropertyMci import PropertyMci
from src.PropertyMci.WorkItem import WorkItem
...
def parse_line_with_state(line: str):
    match FSM_STATE:
        case FsmState.START_COUNTY:
            pass
        case FsmState.START_DOCUMENT:
            pass
        case FsmState.START_COUNTY:
            pass
        case FsmState.START_PROPERTY:
            pass
        case FsmState.UPDATE_DOCKET:
            pass
        case FsmState.ADD_WORK_ITEM:
            pass
        case FsmState.END_PROPERTY:
            pass
        case FsmState.END_COUNTY:
            pass
        case FsmState.ADD_COUNTY_TALLY:
            pass
        case FsmState.END_DOCUMENT:
            pass
...
def is_valid_line_type_and_fsm_state(line_type: LineType) -> bool:
    ...
    if FSM_STATE == FsmState.END_COUNTY:
        return True
    if FSM_STATE == FsmState.ADD_COUNTY_TALLY:
        return True
    if FSM_STATE == FsmState.END_DOCUMENT:
        return True

    if (
        FSM_STATE == FsmState.START_DOCUMENT
        and line_type == LineType.COUNTY_DATE_HEADER
    ):
        FSM_STATE = FsmState.START_COUNTY
        return True

    if FSM_STATE == FsmState.START_COUNTY and line_type == LineType.STREET_ADDRESS_LINE:
        return True

    if FSM_STATE == FsmState.START_PROPERTY:
        return True
    if FSM_STATE == FsmState.ADD_WORK_ITEM and (
        LineType
        in [
